...
const actions=[
{id:"endure",title:"忍了",desc:"学习快，但掉状态疼。",available:()=>true,run:()=>{state.study+=15;state.patience-=15;state.hp-=state.smell*.14;state.sanity-=7;state.story="你忍了。知识进脑，气味入魂。";state.effect="学习 +15，忍耐 -15，精神小降。"}},
...
{id:"mint",title:"薄荷糖",desc:"仅 1 次，回精神。",available:()=>state.mint>0,run:()=>{state.mint-=1;state.hp+=20;state.sanity+=16;state.patience+=4;state.story="你吃了薄荷糖。续命成功。";state.effect="精神 +20，理智 +16，忍耐 +4。"}},
...
{id:"rule",title:"宿舍公约",desc:"宿舍可用，隐藏结局关键。",available:()=>state.scene.id==="dorm"&&!state.hasRule&&state.courage>=24,run:()=>{state.hasRule=true;state.courage-=18;state.relationship+=10;state.remind+=1;state.air+=14;state.smell-=30;state.patience+=8;state.story="你提出宿舍公约：通风、换洗、运动后处理。";state.effect="长期降低宿舍污染，关系 +10，忍耐 +8。"}},
{id:"deepBreath",title:"离场透气",desc:"恢复强，但学习变慢。",available:()=>state.scene.id!=="elevator",run:()=>{state.hp+=22;state.sanity+=14;state.patience+=8;state.study-=6;state.escape+=1;state.story="你离场透气两分钟。灵魂重新上线。";state.effect="精神 +22，理智 +14，忍耐 +8，学习 -6。"}},
...
{id:"quickReview",title:"快速复习",desc:"稳定加学习，轻微掉精神。",available:()=>true,run:()=>{state.study+=16;state.hp-=4;state.patience-=3;state.story="你抓紧时间快速复习，知识点开始回流。";state.effect="学习 +16，精神 -4，忍耐 -3。"}},
{id:"askClassmate",title:"问同学",desc:"学习增加，也能稳住情绪。",available:()=>state.relationship>=15,run:()=>{state.study+=12;state.patience+=5;state.relationship-=2;state.story="你小声问了同学，终于把卡住的点顺过去了。";state.effect="学习 +12，忍耐 +5，关系 -2。"}},
{id:"focusMode",title:"专注模式",desc:"大幅加学习，但掉理智。",available:()=>state.sanity>=18,run:()=>{state.study+=22;state.sanity-=10;state.hp-=5;state.story="你强行进入专注模式，世界安静了，进度上去了。";state.effect="学习 +22，理智 -10，精神 -5。"}},
{id:"nap",title:"小憩回血",desc:"恢复精神，学习略降。",available:()=>state.scene.id!=="elevator",run:()=>{state.hp+=20;state.sanity+=8;state.patience+=4;state.study-=6;state.story="你小憩了一会儿，脑子终于不嗡了。";state.effect="精神 +20，理智 +8，忍耐 +4，学习 -6。"}},
{id:"musicHeal",title:"听歌回血",desc:"恢复精神和忍耐。",available:()=>true,run:()=>{state.hp+=14;state.patience+=8;state.sanity+=5;state.story="你戴上耳机听了半首歌，世界柔和了一点。";state.effect="精神 +14，忍耐 +8，理智 +5。"}},
{id:"buyCoffee",title:"喝咖啡",desc:"精神回升，但理智小降。",available:()=>state.scene.id!=="dorm",run:()=>{state.hp+=18;state.study+=6;state.sanity-=4;state.patience-=2;state.story="你喝了咖啡，精神强行上线。";state.effect="精神 +18，学习 +6，理智 -4，忍耐 -2。"}},
{id:"mentalBuild",title:"心理建设",desc:"恢复忍耐，学习略慢。",available:()=>true,run:()=>{state.patience+=16;state.sanity+=6;state.courage+=3;state.study-=4;state.story="你开始心理建设：这只是公共卫生问题，不是世界末日。";state.effect="忍耐 +16，理智 +6，勇气 +3，学习 -4。"}},
{id:"drinkWater",title:"喝水冷静",desc:"小幅恢复忍耐和精神。",available:()=>true,run:()=>{state.patience+=10;state.hp+=6;state.sanity+=4;state.story="你喝了口水，决定先稳住。";state.effect="忍耐 +10，精神 +6，理智 +4。"}},
{id:"studyBreak",title:"闭眼缓冲",desc:"小幅回血，任何场景可用。",available:()=>true,run:()=>{state.hp+=14;state.sanity+=10;state.patience+=8;state.study-=5;state.story="你闭眼十秒，假装世界很清新。";state.effect="精神 +14，理智 +10，忍耐 +8，学习 -5。"}}];
...
function chooseFourActions(){const available=actions.filter(a=>a.available());const recoverIds=["deepBreath","studyBreak","shower","mint","mentalBuild","drinkWater","nap","musicHeal","buyCoffee"];const patienceIds=["mentalBuild","drinkWater","studyBreak","deepBreath","musicHeal","nap"];const spiritIds=["nap","musicHeal","buyCoffee","deepBreath","studyBreak","mint","shower"];const studyIds=["endure","quickReview","askClassmate","focusMode","buyCoffee"];const handleIds=["gentle","direct","window","move","detergent","rule","spray","mask"];const recoverPool=available.filter(a=>recoverIds.includes(a.id));const patiencePool=available.filter(a=>patienceIds.includes(a.id));const spiritPool=available.filter(a=>spiritIds.includes(a.id));const studyPool=available.filter(a=>studyIds.includes(a.id));const handlePool=available.filter(a=>handleIds.includes(a.id));const chosen=[];if(state.hp<55&&spiritPool.length>0)chosen.push(pick(spiritPool));else if(state.patience<45&&patiencePool.length>0)chosen.push(pick(patiencePool));else if(recoverPool.length>0)chosen.push(pick(recoverPool));if(studyPool.length>0){const st=pick(studyPool);if(!chosen.some(a=>a.id===st.id))chosen.push(st)}if(handlePool.length>0){const h=pick(handlePool);if(!chosen.some(a=>a.id===h.id))chosen.push(h)}shuffle(available).forEach(a=>{if(chosen.length<ACTIONS_PER_ROUND&&!chosen.some(x=>x.id===a.id))chosen.push(a)});return chosen.slice(0,ACTIONS_PER_ROUND)}
...
function endDay(){state.day+=1;state.round=1;state.hp+=14;state.patience+=12;state.courage+=8;state.sanity+=10;state.clean-=3;state.air=Math.max(state.air,50);normalize();if(state.day>MAX_DAY)endGame(getSuccess());else newRound("新的一天。精神、忍耐、勇气和理智恢复了一些。")}
