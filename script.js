...
{id:"studyBreak",title:"闭眼缓冲",desc:"小幅回血，任何场景可用。",available:()=>true,run:()=>{state.hp+=10;state.sanity+=8;state.patience+=8;state.study-=5;state.story="你闭眼十秒，假装世界很清新。";state.effect="精神 +10，理智 +8，忍耐 +8，学习 -5。"}}];
...
function endDay(){state.day+=1;state.round=1;state.hp+=12;state.patience+=10;state.courage+=6;state.sanity+=8;state.clean-=3;state.air=Math.max(state.air,45);normalize();if(state.day>MAX_DAY)endGame(getSuccess());else newRound("新的一天。状态恢复很少，危机继续。")}
